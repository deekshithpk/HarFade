...
% Please edit this lemma suitably
...
%	Let $\{X_i\}$ be an independent collection of random variables with zero mean and variance $\overline{E}$. Let $E_i$ be i.i.d non negative random variables with mean $\overline{E}$. Consider an energy harvesting system with $E_i$ as arrivals and $X_i$ as input to the system. Let $0<\varepsilon<1$ be the required probability of error for the energy harvesting system. Then the channel $W$ which only depends on the input may be decoupled from the energy harvesting system by 
%	\begin{enumerate}
%		\item discounting the probability of error requirement for the channel alone to $\varepsilon - \varepsilon'$ where 
%			\begin{equation}
%				\varepsilon' = .
%			\end{equation}
%		\item Directly feeding the input to the channel as if there were no energy harvesting system. 
%	\end{enumerate}
